...
public class TerminationTests {
  ...
  private World<com.pseuco.np21.Clearing, com.pseuco.np21.Trail> world;
  ...
  static void loadMap() throws IOException, URISyntaxException, NoSuchFieldException {
    ...
    clearingAntsField = com.pseuco.np21.Clearing.class.getDeclaredField("ants");
    clearingAntsField.setAccessible(true);

    // In fasterAnts() we’ll be modifying the amount of available food in
    // clearing C.
    clearingFoodField = com.pseuco.np21.Clearing.class.getDeclaredField("food");
  ...
  private void verifyWalk(final InOrder inOrder, final Recorder recorder,
    ...
    for (int i = 1; i < path.length; i++) {
      ...
      List<com.pseuco.np21.Trail> trailList = from.connectsTo();
      final var trail = trailList.stream().filter(t -> t.to() == to).findAny().get();
  ...
  void enoughFoodAtPickup() {
    ...
    verify(recorder, atMost(3)).select(eq(ant), any(), any(),
  ...
  void enoughFoodAndInterruptAtPickup() {
    ...
    List<com.pseuco.np21.Trail> trailList = clearingC.connectsTo();
    verify(recorder, atMostOnce()).select(eq(ant), same(trailList.get(0)), any(),
        same(SelectionReason.RETURN_IN_SEQUENCE));
  ...
  void antEatenWhileEnoughFood() {
    ...
    List<com.pseuco.np21.Trail> trailList = anthill.connectsTo();
    final var trail = trailList.get(0);
